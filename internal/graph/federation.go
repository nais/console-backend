...
func (ec *executionContext) __resolve_entities(ctx context.Context, representations []map[string]interface{}) []fedruntime.Entity {
	...
	resolveEntity := func(ctx context.Context, typeName string, rep map[string]interface{}, idx []int, i int) (err error) {
		...
		case "App":
			...
			case "findAppByID":
				id0, err := ec.unmarshalNID2githubᚗcomᚋnaisᚋconsoleᚑbackendᚋinternalᚋgraphᚋscalarᚐIdent(ctx, rep["id"])
				if err != nil {
		...
		case "DailyCost":
			resolverName, err := entityResolverNameForDailyCost(ctx, rep)
			if err != nil {
				return fmt.Errorf(`finding resolver for Entity "DailyCost": %w`, err)
			}
			switch resolverName {

			case "findDailyCostByID":
				id0, err := ec.unmarshalNID2githubᚗcomᚋnaisᚋconsoleᚑbackendᚋinternalᚋgraphᚋscalarᚐIdent(ctx, rep["id"])
				if err != nil {
					return fmt.Errorf(`unmarshalling param 0 for findDailyCostByID(): %w`, err)
				}
				entity, err := ec.resolvers.Entity().FindDailyCostByID(ctx, id0)
				if err != nil {
					return fmt.Errorf(`resolving Entity "DailyCost": %w`, err)
				}
		...
		case "Team":
			resolverName, err := entityResolverNameForTeam(ctx, rep)
			if err != nil {
				return fmt.Errorf(`finding resolver for Entity "Team": %w`, err)
			}
			switch resolverName {
...
func entityResolverNameForDailyCost(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		var (
			m   map[string]interface{}
			val interface{}
		...
		return "findDailyCostByID", nil
	}
	return "", fmt.Errorf("%w for DailyCost", ErrTypeNotFound)
}

func entityResolverNameForTeam(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		var (
			m   map[string]interface{}
			val interface{}
